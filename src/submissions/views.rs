...
pub async fn update_one(
    ...
    ()
    // let mut obj = super::db::Entity::find_by_id(id)
    //     .one(&db)
    //     .await
    //     .map_err(|_| (StatusCode::NOT_FOUND, Json("Not found".to_string())))?
    //     .ok_or((StatusCode::NOT_FOUND, Json("Not found".to_string())))?;
    ...
    // obj.last_updated = chrono::Utc::now().naive_utc();

    // match obj.save(&db).await {
    //     Ok(_) => {
    //         let response_obj: super::models::SubmissionReadOne = obj.into();
    ...
    //         Some(_) => Err((
    //             StatusCode::INTERNAL_SERVER_ERROR,
    //             Json("Error updating object".to_string()),
    //         )),
    //         _ => Err((
    //             StatusCode::INTERNAL_SERVER_ERROR,
    //             Json("Server error".to_string()),
